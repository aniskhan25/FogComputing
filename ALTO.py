...
parser.add_argument("-m", "--Method", help = "Offloading Selection Method: 0 for Collective (Proposed), 1 for Random, 2 for Nearest, and 3 for MinResidence")
...
#        print(type,':',lt,',',vt,',',rt,',',pt)
    ...
    def StrategyALTO(this):

	get min Us for V R P

	use same strategy

	lu = Math.sqrt( (beta*(1-xp)*(Math.log(T)/Math.log(2))) / ((k)))
    ...
    def CandidateVeh(this, task):
		...
		ds = this.Delay(task.dsize, this.Latency(d))

		if k in this.seen:

			lu = math.sqrt( (beta*(1-task.xp)*(Math.log(this.T)/Math.log(2))) / ((this.K)));

		else:

			lu = ds/x
			this.seen[k] = [ds/x, 1, ds, this.T]
	...
	u -= (u*k + ds/x) / (k+1); // recompute
	k++;

		if this.seen[selected].K == 1:
				
		this.seen[selected] = new(id, mn, 1, ds, this.T));
		...
		if tmp is not null:
			...
			curr.add(new MyList(id, ds/x, 1, ds, T));
			
			this.seen[v.id] lu = Math.sqrt( (
					beta*(1-xp)*(Math.log(T)/Math.log(2))
					) / 
					((k)));
            d = math.sqrt((this.x - v.x) ** 2 + (this.y - v.y) ** 2)
...
print(' Tasks received (rsu): ', recvRsu)
